...
def find_shortest_path(incidence: sps.lil_matrix):
    """
...
def maze_template(size: int, type: str = "empty") -> np.ndarray:
    ...
    # bludiště s průchodem uprostřed
        ...
        if(size < 8):   # vrátíme prazné bludiště pokud pokud typ bludiště neemá smysl pro danou velikost
            return maze
        maze[size // 2, :] = True
        maze[size // 2, (size // 2) - 2 :(size // 2) + 2] = False
        return maze
...
def save_maze(maze: np.ndarray, name: str = "saved_maze.csv"):
    """
